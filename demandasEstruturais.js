...
export function renderDemandaEstruturalDetailPage(demandaId, devedorId) {
  ...
  // Alteração 2: Título da página simplificado
  pageTitle.textContent = "Demandas Estruturais";
  document.title = "SASIF | Demandas Estruturais";
  ...
  contentArea.innerHTML = `
        <div class="detail-page-header"><h2>${
          devedor?.razaoSocial
        }</h2><p>CNPJ: ${devedor?.cnpj}</p></div>
        ...
        <div id="tab-content" class="tab-content">Carregando...</div>
    `;
  ...
  container.innerHTML = `
        <div class="description-card">
            <div class="description-header">
                <h3>Descrição Geral da Demanda</h3>
                <button class="action-icon" data-action="edit-descricao-geral" title="Editar Descrição"><svg viewBox="0 0 24 24"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg></button>
            </div>
    ...
    demanda.descricaoGeral || "Nenhuma descrição geral cadastrada."
        ...
        <p class="empty-list-message">Outras informações de visão geral podem ser adicionadas aqui no futuro.</p>
    `;
}
...
function renderAtoresTab(demanda) {
  ...
  const editAtoresIconFill = activeDemandaState.isEditingAtores
    ...
    : "#555";
  container.innerHTML = `
        <div class="atores-card ${
          activeDemandaState.isEditingAtores ? "edit-mode" : ""
        }">
            <div class="atores-header">
                <h3>Atores Envolvidos</h3>
                <div class="eixos-actions">
                    <button class="action-icon" data-action="add-ator" title="Adicionar Ator"><svg viewBox="0 0 24 24" fill="#4CAF50"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg></button>
            ...
            <div id="atores-list-container"></div>
        </div>
    `;
  ...
  container.innerHTML = `
        <div class="atores-card">
            <div class="atores-header">
                ...
                <div class="eixos-actions">
                    <button class="action-icon" data-action="add-evento" title="Adicionar Evento"><svg viewBox="0 0 24 24" fill="#4CAF50"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg></button>
...
// ==================================================================
// FUNÇÕES DE LÓGICA INTERNA (Atores, Eixos, Descrições, Eventos)
// ==================================================================
...
function renderEventoModal(evento = null) {
  ...
  modalOverlay.innerHTML = `<div class="modal-content modal-large"><h3>${
    ...
    evento?.descricao || ""
  }</textarea></div><div id="error-message"></div><div class="form-buttons"><button data-action="save-evento" data-evento-id="${
    evento?.id || ""
  }" class="btn-primary">Salvar</button><button data-action="close-modal" class="btn-secondary">Cancelar</button></div></div>`;
  document.body.appendChild(modalOverlay);
}
async function handleSaveEvento(eventoId = null) {
  ...
  if (eventoId) {
    eventos = eventos.map((e) =>
      e.id === eventoId ? { ...e, titulo, data, tipo, descricao } : e
    );
  } else {
    const novoEvento = {
      id: `_${Math.random().toString(36).substr(2, 9)}`,
