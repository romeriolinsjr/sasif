...
function renderSidebar(activePage) {
    const pages = [{ id: 'dashboard', name: 'Dashboard' }, { id: 'diligencias', name: 'Diligências Mensais' }, { id: 'grandesDevedores', name: 'Grandes Devedores' }, { id: 'exequentes', name: 'Exequentes' }, { id: 'motivos', name: 'Motivos de Suspensão' }];
...
function renderAnalisePendenteWidget(devedores) {
    ...
    const devedoresParaAnalise = devedores.map(devedor => {
        return { ...devedor, analise: getAnaliseStatus(devedor) };
...
function renderExequentesPage() {
    ...
    contentArea.innerHTML = `<div class="dashboard-actions"><button id="add-exequente-btn" class="btn-primary">Cadastrar Novo Exequente</button></div><h2>Lista de Exequentes</h2><div id="exequentes-list-container"></div>`;
...
function renderDiligenciasPage() {
    ...
    `;
    document.getElementById('add-diligencia-btn').addEventListener('click', () => renderDiligenciaFormModal());
...
function renderDiligenciasList(diligencias) {
    ...
    diligencias.forEach(item => {
        ...
        if (cumpridoEsteMes) {
            const dataFormatada = item.historicoCumprimentos[chaveMesAno].toDate().toLocaleDateString('pt-BR');
            statusHTML = `<span class="status-badge status-cumprido">Cumprido em ${dataFormatada}</span>`;
            acoesHTML = `<button class="action-btn btn-desmarcar" data-action="desmarcar">Desfazer</button>`;
        } else {
            statusHTML = `<span class="status-badge status-pendente">Pendente</span>`;
            acoesHTML = `<button class="action-btn btn-marcar-cumprido" data-action="marcar-cumprido">Cumprir Hoje</button>`;
        }
...
function renderProcessoDetailPage(processoId) {
    ...
    db.collection("processos").doc(processoId).get().then(doc => {
        ...
        document.getElementById('back-to-devedor-btn').addEventListener('click', () => { renderDevedorDetailPage(processo.devedorId); });
...
function handleDevedorAction(event) {
    ...
    if (target.classList.contains('action-btn')) {
        ...
        if (target.classList.contains('btn-delete')) handleDeleteDevedor(devedorId);
        else if (target.classList.contains('btn-edit')) handleEditDevedor(devedorId);
    } else {
        const row = target.closest('tr');
        if (row && row.dataset.id) renderDevedorDetailPage(row.dataset.id);
    }
}
...
function handleDeleteDevedor(devedorId) {
    if (confirm("Tem certeza que deseja excluir este Grande Devedor?")) db.collection("grandes_devedores").doc(devedorId).delete().then(() => showToast("Devedor excluído com sucesso.")).catch(() => showToast("Ocorreu um erro ao excluir.", "error"));
}
...
function handleEditProcesso(processoId) {
    const processo = processosCache.find(p => p.id === processoId);
    if (processo) renderProcessoForm(processo.devedorId, processo);
    else showToast("Processo não encontrado.", "error");
}

function handleDeleteProcesso(processoId) {
    const processo = processosCache.find(p => p.id === processoId);
    if (confirm(`Tem certeza que deseja excluir o processo ${formatProcessoForDisplay(processo.numeroProcesso)}?`)) db.collection("processos").doc(processoId).delete().then(() => showToast("Processo excluído com sucesso.")).catch(() => showToast("Ocorreu um erro ao excluir.", "error"));
}

function handleRegistrarAnalise(devedorId) {
    if (confirm("Deseja registrar a data de análise para hoje?")) {
        db.collection("grandes_devedores").doc(devedorId).update({ dataUltimaAnalise: firebase.firestore.FieldValue.serverTimestamp() }).then(() => showToast("Data de análise registrada com sucesso!")).catch(err => {
...
function renderDevedorForm(devedor = null) {
    ...
    document.getElementById('cancel-btn').addEventListener('click', () => navigateTo('grandesDevedores'));
}
...
function getDevedorDataFromForm() {
    ...
    return { razaoSocial, cnpj: cnpj.replace(/\D/g, ''), nomeFantasia: document.getElementById('nome-fantasia').value, nivelPrioridade: parseInt(document.getElementById('nivel-prioridade').value), observacoes: document.getElementById('observacoes').value };
}
...
function handleSaveDevedor() {
    ...
    db.collection("grandes_devedores").add(devedorData).then(() => {
...
function handleUpdateDevedor(devedorId) {
    ...
    db.collection("grandes_devedores").doc(devedorId).update(devedorData).then(() => {
        navigateTo('grandesDevedores');
...
function handleSaveExequente() {
    ...
    const data = { nome, cnpj: cnpjInput.replace(/\D/g, ''), criadoEm: firebase.firestore.FieldValue.serverTimestamp() };
...
function handleUpdateExequente(exequenteId) {
    ...
    const data = { nome, cnpj: cnpjInput.replace(/\D/g, ''), atualizadoEm: firebase.firestore.FieldValue.serverTimestamp() };
...
function handleDeleteExequente(exequenteId) {
    if (confirm("Tem certeza que deseja excluir este Exequente?")) {
        db.collection("exequentes").doc(exequenteId).delete().then(() => showToast("Exequente excluído com sucesso.")).catch(() => showToast("Ocorreu um erro ao excluir.", "error"));
    }
}
